...
Always registered beside them, though not Go's: `toString` and `toStrings` (sprig's `strval`/`strslice`) — Go's `%v` with one argument, printed through the engine's `toString`/`isT` like `printf` is.
...
| Conversions | `sprigConversions()` | atoi, int, int64, float64, toDecimal, toRawJson (`toString`/`toStrings` are Go's `%v` and live with the built-ins) |
...
- **String slicing.** `trunc`, `substr`, `abbrev`, and `abbrevboth` count code points where Go counts bytes, so the same `n` keeps a different amount of text in each engine: `substr 0 2 "éx"` → `é` in Go, `éx` here. The divergence starts at any two-byte character, not only astral ones, and Go's answer is frequently not valid UTF-8 at all: `trunc 1 "éx"` → the single byte `0xC3` in Go, `é` here; `trunc 1 "𐍈x"` → the single byte `0xF0` in Go, `𐍈` here. Each of those Go results is the first fragment of the character's UTF-8 encoding, and is not a well-formed string on its own. Counting code points is deliberate: it keeps them from splitting a character and returning a lone surrogate. Separately, `abbrevboth` disagrees with Go on plain ASCII whenever the offset is 4 or less — a defect rather than an intended residual, and one that will be fixed.
- **Width counting.** `wrap` and `wrapWith` measure width in UTF-16 code units where Go measures bytes: `wrap 5 "éé éé"` → `éé\néé` in Go, unwrapped `éé éé` here. `printf`'s width flags diverge on a different axis — Go pads to a rune count and this engine to a UTF-16 unit count, so the two agree on BMP text and differ only on astral: `printf "%-5s|" "𐍈𐍈"` → `𐍈𐍈   |` in Go, `𐍈𐍈 |` here. Neither is converted to code points: they never cut inside a character, so they cannot emit a lone surrogate, and counting runes in `wrap` would move it further from Go's bytes rather than closer.
...
- **Splitting by character** matches Go. `split ""`, `splitList ""`, and `trimAll`'s cutset all work in whole characters on both sides, astral input included — pinned by the conformance fixtures `sprig-split-astral-empty-sep`, `sprig-splitlist-astral-empty-sep`, and `sprig-trimall-astral-cutset`. It is the slicing and width-counting functions, noted above, that do diverge.
