...
describe("_duration.formatDurationNs", () => {
  ...
  it("formats minutes with 0 seconds", () => expect(formatDurationNs(secondsToNs(60))).toBe("1m0s"));
  it("formats hours+minutes+seconds", () => expect(formatDurationNs(secondsToNs(3661))).toBe("1h1m1s"));
...
describe("_duration.roundDurationNs", () => {
  it("rounds 1h30m to 2h", () =>
    expect(roundDurationNs(parseDurationNs("1h30m"))).toBe("2h0m0s"));
  it("rounds 1h29m to 1h", () =>
    expect(roundDurationNs(parseDurationNs("1h29m"))).toBe("1h0m0s"));
  it("rounds 90s to 2m", () =>
    expect(roundDurationNs(parseDurationNs("90s"))).toBe("2m0s"));
  ...
  // 29s < 1m → stays 29s
  it("29s stays 29s", () =>
    expect(roundDurationNs(parseDurationNs("29s"))).toBe("29s"));
});
